...
usage() {
    echo ""
    echo "Usage: sudo -E bash HBaseRecoveryScript_secure.sh [storage-account-name] [number of worker nodes]" ;
...
remove_zk_data()
{
  ...
  echo 'rmr /hbase-unsecure' | ./zkCli.sh -server $HOSTNAME:2181 -n
  status_code=$?
...
delete_current_wal_directory() 
{
  ...
  if ! hdfs dfs -rm -r hdfs://mycluster/hbasewal; then
...
refresh_hbase()
{
  ...
  do
    i=${i:6}
    hdfs dfs -mkdir -p hdfs://mycluster/hbasewal/WALs/$i;
  done
...
while true 
do
  echo 'scan "hbase:meta", { COLUMNS => ["info:sn"] }' | hbase shell -n > metatable.txt
