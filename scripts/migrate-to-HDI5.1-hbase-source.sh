...
#----------------------------------------------------------------
# Hbase Cluster Health Check
hbase_health_check(){
	# check if hbase hbck has any inconsistencies
	echo "Checking if HBase has any inconsistencies."
	hbase hbck > $TMP_FILE_DIR/hbase_hbck.out 2> $TMP_FILE_DIR/hbase_hbck.err
	if [ $? -ne 0 ]; then
		echo "Error executing hbase hbck. Exiting."
		exit 1
	fi
	grep "Status: OK" $TMP_FILE_DIR/hbase_hbck.out
	if [ $? -ne 0 ]; then
		echo "HBase has inconsistencies. Exiting."
		exit 1
	fi

	# check if hbase has any procedure in progress
	echo "Checking if HBase has any procedure in progress."
	hbase shell <<< "list_procedures" > $TMP_FILE_DIR/hbase_list_procedures.out 2> $TMP_FILE_DIR/hbase_list_procedures.err
	if [ $? -ne 0 ]; then
		echo "Error executing hbase list_procedures. Exiting."
		exit 1
	fi
	grep "0 row(s)" $TMP_FILE_DIR/hbase_list_procedures.out
	if [ $? -ne 0 ]; then
		echo "HBase has procedures in progress. Exiting. Please wait for them to complete and try again."
		exit 1
	fi

	# check if hbase has replication in progress
	echo "Checking if HBase has replication in progress."
	hbase shell <<< "list_peers" > $TMP_FILE_DIR/list_peers.out 2> $TMP_FILE_DIR/list_peers.err
	if [ $? -ne 0 ]; then
		echo "Error executing hbase list_peers. Exiting."
		exit 1
	fi
	grep "0 row(s)" $TMP_FILE_DIR/list_peers.out
	if [ $? -ne 0 ]; then
		echo "HBase has replication in progress. Exiting. Please stop replication before starting migration."
		exit 1
	fi
...
#----------------------------------------------------------------
# Function to get IsActiveMaster value from JSON response
get_is_active_master() {
  local json_response=$1
  local is_active_master=$(echo "$json_response" | grep -oP '(?<="IsActiveMaster"\s*:\s*)\w+')
  echo "$is_active_master"
}


#----------------------------------------------------------------
# Function to get Hmaster list
get_hmasters(){

	HOSTNAMES=( $(curl -u $AMBARI_USER:$AMBARI_PASSWORD -i -X GET https://$CLUSTER.azurehdinsight.net/api/v1/clusters/$CLUSTER/services/HBASE/components/HBASE_MASTER -H "X-Requested-By:ambari" 2>/dev/null |grep host_name |awk '{print $3}'|tr -d '"') )

	# Array to hold the sorted hostnames
	sorted_hostnames=()

	# Loop through each hostname
	for hostname in "${HOSTNAMES[@]}"; do
	# Curl the API and get JSON response
	json_response=$(curl -u $AMBARI_USER:$AMBARI_PASSWORD -i -X GET "https://$CLUSTER.azurehdinsight.net/api/v1/clusters/$CLUSTER/hosts/$hostname/host_components/HBASE_MASTER" -H "X-Requested-By:ambari" 2>/dev/null)

	# Get IsActiveMaster value
	is_active_master=$(get_is_active_master "$json_response")

	# Append hostname to sorted_hostnames based on IsActiveMaster value
	if [ "$is_active_master" == "true" ]; then
		sorted_hostnames+=("$hostname")
	else
		sorted_hostnames=("$hostname" "${sorted_hostnames[@]}")
	fi
	done
}
...
stop_hbase()
{
	...
	if [[ $HBASESTATE != "INSTALLED" ]]; then
		get_hmasters
	    for hmaster in "${sorted_hostnames[@]}"; do
			echo "Stopping HBase on $hmaster"
			curl -u $AMBARI_USER:$AMBARI_PASSWORD -i -X PUT -d '{"RequestInfo" : {"context" : "Stop HBASE via REST"}, "Body" : {"HostRoles":{"state":"INSTALLED"}}}' https://$CLUSTER.azurehdinsight.net/api/v1/clusters/$CLUSTER/hosts/$hmaster/host_components/HBASE_MASTER -H "X-Requested-By:ambari" -o $TMP_FILE_DIR/hmaster.json 2> $TMP_FILE_DIR/hmaster.err
			if [ $? -ne 0 ]; then
				echo "Error: Failed executing curl to stop HMaster on $hmaster."
				exit 1
			fi

			grep -q "Accepted" $TMP_FILE_DIR/hmaster.json # This will fail if accepted is not found when we are running with set -e
			if [ $? -ne 0 ]; then
				echo "Error: The curl command to stop HMaster on $hmaster was not accepted."
				exit 1
			fi
			sleep 3
